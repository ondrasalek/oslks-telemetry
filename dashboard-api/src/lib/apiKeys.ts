...
const KEY_PATTERN = new RegExp(
    `^${KEY_NAMESPACE}_([0-9a-f]{${PREFIX_BYTES * 2}})_([0-9a-f]{${SECRET_BYTES * 2}})$`,
);
...
export const parseApiKey = (token: string): { prefix: string } | null => {
    const match = KEY_PATTERN.exec(token);
    return match ? { prefix: match[1] as string } : null;
};
